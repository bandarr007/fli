...
local ZkChengUserName = database:get(bot_id.."Cheng:UserName"..data.id_)
if not data.username_ then 
if ZkChengUserName then 
send(msg.chat_id_, msg.id_, 1, "حذف معرفه خمطو بساع بساع  \n هاذه معرفه  : [@"..ZkChengUserName..']')
database:del(bot_id.."Cheng:UserName"..data.id_) 
end
end
if data.username_ then 
if ZkChengUserName ~= data.username_ then 
local Text = {
